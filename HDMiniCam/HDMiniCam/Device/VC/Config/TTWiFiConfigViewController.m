...
- (void)addNoti
{
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(getDeviceWiFi:) name:TT_getDeviceWiFiCmdResult_noti_KEY object:nil];
}
...
- (void)getDeviceWiFi:(NSNotification *)noti
{
    ...
    if (ret == 0) {
        ...
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(searchDeviceWiFi:) name:TT_getSearchDeviceWiFi_noti_KEY object:nil];
    }
    else {
