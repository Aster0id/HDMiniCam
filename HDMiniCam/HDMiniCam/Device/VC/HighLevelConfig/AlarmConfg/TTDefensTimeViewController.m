...
#import "TTAddDefensTimeViewController.h"

@interface TTDefensTimeViewController ()
<
UITableViewDelegate,
UITableViewDataSource,
TTAlarmConfigHeadViewDelegate,
TTAlarmConfigFootViewDelegate,
TTDefensTimeCellDelegate,
TTAddDefensTimeViewControllerDelegate
>
...
@implementation TTDefensTimeViewController
...
{
    if (!isEveryDay) {
        TTAlarmConfigFootView *head = [[NSBundle mainBundle] loadNibNamed:@"TTAlarmConfigFootView" owner:nil options:nil][0];
        head.tag = FLAG_TAG * 2;
        head.delegate = self;
        return head;
    }
...
{
    TTDefensTimeCell *cell = [tableView dequeueReusableCellWithIdentifier:@"TTDefensTimeCell"];
    if (!cell) {
        cell = [[NSBundle mainBundle] loadNibNamed:@"TTDefensTimeCell" owner:nil options:nil][0];
    }
...
#pragma mark - TTAlarmConfigFootViewDelegate

- (void)clickFootWith:(NSInteger)index
{
    TTAddDefensTimeViewController *vc = [[TTAddDefensTimeViewController alloc] init];
    vc.delegate = self;
    if (index == 1)
...
#pragma mark - TTAddDefensTimeViewControllerDelegate
...
#pragma mark - TTDefensTimeCellDelegate
