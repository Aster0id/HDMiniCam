...
- (BOOL)textField:(UITextField *)textField shouldChangeCharactersInRange:(NSRange)range replacementString:(NSString *)string
{
    if ([string isEqualToString:@""]) { // 删除字符
        ...
        if (_intervalNum > 0) {
            ...
            if ((textField.text.length + 1) % (_intervalNum + 1) == 0
...
- (void)setEnsureBgColor:(UIColor *)ensureBgColor
{
    ...
    self.cancelBtn.backgroundColor = ensureBgColor;
}
...
- (void)show
{
    ...
    CGSize size = [TTCommon sizeWithText:self.titleLabel.text maxSize:CGSizeMake(250, MAXFLOAT)];
...
{
    if (self.needPermissionView && [_textField isFirstResponder]) {
        [self endEditing:YES];
    }
    else {
        [self dismiss];
...
- (IBAction)ensureBtnClick:(UIButton *)sender
{
    [self dismiss];
    if (self.ensureBlock) {
        if (_needPermissionView) {
            UIImageView *image1 = [self viewWithTag:10];
            if (image1.hidden == NO) {
                self.ensureBlock(self.textField.text, 0);
            }
            else {
                self.ensureBlock(self.textField.text, 1);
            }
        }
        else {
            self.ensureBlock(self.textField.text, FLAG_TAG);
        }
    }
}
...
- (IBAction)choosePermissionAction:(UIButton *)sender
{
    UIImageView *image1 = [self viewWithTag:10];
    UIImageView *image2 = [self viewWithTag:11];
    if (sender.tag == 10) {
        image1.hidden = NO;
        image2.hidden = YES;
    }
